...
public final class PoTransaction {
     ...
     * <p>
     * If the {@link PoCommand} list is null or empty, nothing is added to the {@link ApduRequest}
     * list and returned split command index is -1
     * <p>
     ...
     */
    private int buildApduRequests(List<PoCommand> poCommands, List<ApduRequest> apduRequestList) {
        if (poCommands != null) {
    ...
    private static class AnticipatedResponseBuilder {
         ...
         */
        static void storeCommandResponse(List<PoCommand> poCommands, List<ApduRequest> apduRequests,
                List<ApduResponse> apduResponses, int first, int last) {
            if (poCommands != null) {
                /*
                        ...
                        if (commandResponse != null) {
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        ...
                                        (poCommand.getCommandBuilder() instanceof DecreaseCmdBuild)
                                                ? "Decrease"
                                                : "Increase",
    ...
    public boolean processPoCommands(ChannelControl channelControl)
        ...
        if (!createResponseParsers(seResponsePoCommands, poCommandManager.getPoCommandList())) {
            poProcessSuccess = false;
        }
     ...
     * @throws KeypleCalypsoSecurityException if the cryptographic computations show a problem
     */
    public boolean processPoCommandsInSession()
            throws KeypleReaderException, KeypleCalypsoSecurityException {
     ...
     * @throws KeypleCalypsoSecurityException if the cryptographic computations show a problem
     */
    public boolean processClosing(ChannelControl channelControl)
            throws KeypleReaderException, KeypleCalypsoSecurityException {
        boolean poProcessSuccess = true;
     ...
     */
    private boolean createResponseParsers(SeResponse seResponse, List<PoCommand> poCommands) {
